...
const SignIn = ({ onNavigate }) => {
  ...
  const onSubmit = async (vals) => {
    ...
    if (response.ok) {
      ...
      // Corrected redirect logic for all user roles
      if (data.user.role === 'shop') {
        ...
        onNavigate('landing'); // Fallback for any other roles
      }
    } else {
    ...
    <div className="flex items-center justify-center min-h-screen p-4">
      <div className="w-full max-w-sm bg-white rounded-2xl shadow-2xl p-8">
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <Button type="submit" disabled={Object.keys(errors).length > 0}>Sign In</Button>
        </form>
        <div className="mt-6 text-center">
          <p className="text-sm text-gray-600">
            Don't have an account?{' '}
            <button onClick={() => onNavigate('signup')} className="text-lime-600 hover:underline font-semibold">
              Sign Up
            </button>
          </p>
        </div>
